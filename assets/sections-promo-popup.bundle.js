  ...
  Alpine.data('ThemeSection_PromoPopup', settings => {
    return {
      ...
      mounted() {
        ...
        if (popupContent && popupContent.querySelectorAll('.errors, .form-message').length > 0 && this.cookie !== 'opened') {
          this.open();
        }
        if (popupContent && popupContent.querySelectorAll('.form-message').length > 0 && this.cookie !== 'opened') {
          api.set('promoPopup', 'opened', {
            path: '/',
            expires: 200
          });
